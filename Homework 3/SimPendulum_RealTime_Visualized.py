...
screen = pygame.display.set_mode((SCREEN_SIZE, SCREEN_SIZE))        # Create a blank screen of defined size
pygame.display.set_caption('Simple Pendulum Simulation')            # To set caption on screen

PIVOT = (SCREEN_SIZE/2, SCREEN_SIZE/9)                              # Pivot point to hang the bob
l = PIVOT[1] * 4                                                    # Length of the string
...
font = pygame.font.SysFont('Arial', 25)                             # For timer on pygame simultaion       
...
class RealTime():
    ...
    def run(self):
        for j in range(self._te):
            ...
            f1 = (-self._mu * self._om[j] + self._m * self._g * self._l * sin(self._th[j]) + self._u)/(self._m*(self._l^2))
            self._om.append(self._om[j] + self._ts * f1)
            
            # Approximation 1 at mid-interval
            th2 = self._th[j+1] + (self._ts/2) * self._om[j+1]
            f2 = (-self._mu * self._om[j+1] + self._m * self._g * self._l * sin(self._th[j+1]) + self._u)/(self._m*(self._l^2))
            om2 = self._om[j+1] + (self._ts/2) * f2
            
            # Approximation 2 at mid-interval
            th3 = th2 + (self._ts/2) * om2
            f3 = (-self._mu * om2 + self._m * self._g * self._l * sin(th2) + self._u)/(self._m * (self._l^2))
            om3 = om2 + (self._ts/2) * f3
            
            # Approximation at next time step
            th4 = th3 + (self._ts) * om3
            f4 = (-self._mu * om3 + self._m * self._g * self._l * sin(th3) + self._u)/(self._m * (self._l^2))
            om4 = om3 + (self._ts) * f4
    ...
    def Animate(self, theta):
        while True:
            ...
            pos = self.getPosition(theta.next())                 # RK4 acts as a generator and using next() subsequent angles can be obtained
            screen.fill(black)
            time_clock = pygame.time.get_ticks()/1000       # Converting milliseconds to seconds
            format_time = '{0:02d}'.format(time_clock)
            screen.blit(font.render('Time (s): ' + format_time, True, green), (400, 20))
            pygame.draw.lines(screen, white, False, [(0,PIVOT[1]), (SCREEN_SIZE, PIVOT[1])], 2)
            pygame.draw.circle(screen, red, PIVOT, 3, 3)
            pygame.draw.lines(screen, red, False, [PIVOT, (pos[0],pos[1])], 2)
            pygame.draw.circle(screen, green, (pos[0],pos[1]), BOB_SIZE, BOB_SIZE)
            pygame.display.flip()
